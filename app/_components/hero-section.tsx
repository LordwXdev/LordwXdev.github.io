...
const Hero = () => {
  ...
  return (
    <section className="relative w-full flex items-center bg-background overflow-hidden pt-[0.5cm]">
      <div className="w-full grid grid-cols-1 lg:grid-cols-2 items-center lg:items-end gap-12 px-6 md:px-12 lg:px-20 pb-20">
        ...
        <div className="relative z-20 text-center lg:text-left">
          ...
          {/* BUTTONS: hover:text-orange-500 ensures it turns orange in dark mode */}
        ...
        >
          {/* Background Arc: dark:bg-orange-500/10 keeps it subtle in dark mode */}
          ...
          {/* Doodles (Visible on Hover/Touch) */}
          <AnimatePresence>
            {hovered && (
              <motion.img
          ...
          {/* MAIN PORTRAIT: Stable positioning (2cm Shifted) */}
          <img
  ...
  <motion.div 
    ...
    animate={{ 
        opacity: 1, 
        y: hovered ? -10 : 0,
        x: "60%" // Keeps it centered relative to the avatar
    }}
    /* top-[0px] pushes it above the head of the photo */
    className="absolute top-[250px] left-1/2 z-30 hidden xl:block"
  >
   <div className="flex gap-1 justify-end text-orange-500 mb-2">
                {[...Array(5)].map((_, i) => <span key={i} className="text-xl">★</span>)}
             </div>
             <div className="text-2xl font-black text-slate-900">Early, Hungry</div>
  ...
  <AnimatePresence>
    {hovered && (
      ...
      />
          ...
          {/* SOCIALS: hover:text-orange-500 works in both Light/Dark modes */}
          <div className="absolute bottom-[-60px] lg:bottom-[-80px] flex gap-4 lg:gap-6 px-6 lg:px-8 py-4 z-20 glass -translate-x-[1cm]">
            {socials.map((s, i) => (
              <a
                key={i}
                href={s.link}
                className="w-9 h-9 lg:w-10 lg:h-10 flex items-center justify-center rounded-full bg-background/50 text-muted-foreground hover:text-orange-500 hover:scale-110 transition-all duration-300"
              >
                {s.icon}
              </a>
            ))}
